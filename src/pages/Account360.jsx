...
const Account360 = () => {
  ...
  const handleEnrichment = async () => {
    ...
    try {
      await enrichmentService.triggerDataEnrichment(id, 'account');
      setEnrichmentStatus('Enriched');
    } catch (e) {
      setEnrichmentStatus('Unverified');
    } finally {
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm mb-6">
            <h4 className="text-sm font-black text-slate-900 mb-4 flex items-center justify-between">
              ...
              <span className={`text-[10px] uppercase font-black px-2 py-1 rounded ${enrichmentStatus === 'Enriched' ? 'bg-emerald-100 text-emerald-700' : enrichmentStatus === 'Syncing...' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
                {enrichmentStatus}
              </span>
            </h4>
